...
function sauvegarderLecturesEnCours(lectures, options = {}) {
  if (!window.localStorage) return;
  localStorage.setItem(LECTURES_EN_COURS_KEY, JSON.stringify(lectures));
  if (!options.skipCloud && window.__pousserLecturesVersCloud) {
    window.__pousserLecturesVersCloud(lectures);
  }
}
...
async function chargerDonnees() {
  const reponse = await fetch(`${API_URL}/data`, { cache: "no-store" });
...
async function ajouterSerie(donnees, { titre, synopsis, miniature, genres, affiche, carrousel, type, videoUrl }) {
  const reponse = await fetch(`${API_URL}/series`, {
    method: "POST",
    headers: entetesAdmin(),
    body: JSON.stringify({ titre, synopsis, miniature, genres, affiche, carrousel, type, videoUrl }),
  });
...
async function modifierSerie(donnees, serieId, { titre, synopsis, miniature, genres, affiche, carrousel, type, videoUrl }) {
  ...
  if (serie) {
    ...
    serie.carrousel = Boolean(serieModifiee.carrousel);
